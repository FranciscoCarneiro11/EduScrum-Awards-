...
export default function Rankings() {
    ...
    // 1. Carregar Ranking Alunos + Lista de Projetos (ao iniciar)
    useEffect(() => {
        async function fetchDataInicial() {
            try {
                ...
                // (Nota: Assumindo que existe um endpoint que lista todos os projetos. 
                // Se não existir, teremos de criar ou buscar por curso)
                try {
                    const resProjetos = await api.get<ProjetoSimple[]>("/api/projetos")
                    setProjetos(resProjetos.data)
                } catch (e) {
                    console.warn("Não foi possível carregar projetos (verificar endpoint GET /api/projetos)")
                }
    ...
    // 2. Carregar Ranking de Equipas quando se escolhe um Projeto
    useEffect(() => {
        ...
        async function fetchRankingEquipas() {
            try {
                ...
                setRankingEquipas([]) // Limpa se der erro
            } finally {
        ...
        <div className="min-h-screen bg-gray-50 px-6 py-10">
            <div className="max-w-4xl mx-auto">
                ...
                <div className="text-center mb-10">
                    <h1 className="text-4xl font-bold bg-gradient-to-r from-yellow-500 to-orange-500 bg-clip-text text-transparent inline-flex items-center gap-3">
                ...
                <Tabs defaultValue="alunos" className="w-full">
                    ...
                    <TabsContent value="alunos">
                        <Card className="shadow-lg border-t-4 border-t-yellow-400">
                        ...
                        `}
                                        >
                                            ...
                                            <div className="flex-shrink-0 w-10 h-10 rounded-full bg-gradient-to-br from-violet-500 to-purple-600 flex items-center justify-center text-white font-bold ml-4 shadow-sm">
                                                {aluno.nome.charAt(0)}
                                            </div>
                                            ...
                                            <div className="flex items-center gap-2 bg-violet-100 px-3 py-1 rounded-full border border-violet-200">
                                                <Star className="w-4 h-4 text-violet-600 fill-current" />
                                                <span className="font-bold text-violet-700">{aluno.totalPontos} pts</span>
                                            </div>
                                        </div>
                                    ))
                                ) : (
                    ...
                    <TabsContent value="equipas">
                        <Card className="shadow-lg border-t-4 border-t-blue-400 min-h-[300px]">
                            <CardContent className="p-6">
                                ...
                                <div className="mb-6">
                                    ...
                                    <select
                                        className="w-full p-2 border border-gray-300 rounded-md bg-white focus:ring-2 focus:ring-blue-500 outline-none transition"
                                ...
                                {!selectedProjetoId ? (
                                    ...
                                    <div className="space-y-3">
                                        {rankingEquipas.map((equipa, index) => (
                                            <div
                                                key={equipa.idEquipa}
                                                className="flex items-center justify-between p-4 bg-white border rounded-xl hover:shadow-md transition relative overflow-hidden group"
                                            >
                                                <div className={`absolute left-0 top-0 bottom-0 w-1 
                                    ${index === 0 ? 'bg-yellow-500' : index === 1 ? 'bg-gray-400' : index === 2 ? 'bg-amber-600' : 'bg-transparent'}
                                `}></div>
                                                ...
                                                <div className="flex items-center gap-4">
                                                    ...
                                                    <div>
                                                        <h3 className="font-bold text-gray-800 text-lg">{equipa.nomeEquipa}</h3>
                                                ...
                                                <div className="text-right">
                                                    ...
                                                    <span className="text-xs text-blue-400 font-medium uppercase tracking-wider">Média / Aluno</span>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
